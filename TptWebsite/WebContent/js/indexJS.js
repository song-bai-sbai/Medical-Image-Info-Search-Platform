...
function submitForm() {
	...
	data[getEleName("query")] = getQuery();
	var jsonString = JSON.stringify(data);

	var request = createXmlHttpRequest();// create request
	request.open("post", "parseSelection?selection=" + jsonString, true);
...
function openListWindow(content) {
	...
	if (document.all) // IE
	{
		...
		feature = "width=500,height=500,menubar=no,toolbar=no,location=no,";
